...
## Proposed Changes (Room UI & Flow Refinement Phase)

### 3D-Brawl [MODIFY]
- **Main Menu UI**:
    - 初期画面からパスワード入力欄を削除。
    - 「ルームを作る」「ルームに入る」「NPCと対戦」の3ボタン構成に変更。
- **Room Overlays**:
    - **Create Room Overlay**: パスワード設定用の入力欄を表示。「対戦待ち」状態への遷移。
    - **Join Room Overlay**: 接続先パスワードの入力欄を表示。「接続試行」状態への遷移。
- **Explicit Connection Logic**:
    - **作成時**: 必ず自分をLobby(Host)として初期化。IDが重複している場合はエラー通知。
    - **参加時**: 指定されたパスワードのLobbyに接続試行。Lobbyが見つからない場合はエラー通知。
